...
export default function DoctorsComponent(): ReactElement {
  ...
  const { currentItems, currentPage, totalPages, setPage } = usePagination({ items: doctors, pageSize: 7 });
  ...
  return (
    <section className={styles["doctor-container"]} >
      {currentItems.map(doctor => <DoctorCardComponent key={doctor.id} doctor={doctor} />)}
      <div className={styles.pagination}>
        {[...Array(totalPages)].map((_, i) => (
          <button
            key={i + 1}
            onClick={() => setPage(i + 1)}
            className={clsx(styles.paginationBtn, `${i + 1 === currentPage ? styles.active : ''}`)}
          >
            {i + 1}
          </button>
        ))}
